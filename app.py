...
app.config['JWT_SECRET_KEY'] = os.environ.get('JWT_SECRET_KEY', 'e173db52f146f6d5e957a922')
...
class Users(Resource):
    ...
    def post(self):
        ...
        if not name or len(name) < 3:
            errors.append("Name is required and should be at least 3 characters")
        if len(name)<2:
        ...
        if not password or len(password) < 8:
            errors.append('Password is required and should be at least 8 characters')
...
class Parcels(Resource):
    ...
    def post(self):
        ...
        if not sender or not recipient or not location or not vehicle:
            return make_response({
                "error":"Please enter a valid sender,recipient,vehicle or location details"
            },400)
            
        
        if not description or not weight or not name or weight <= 0:
            return make_response({
...
class Vehicles(Resource):
    ...
    def post(self):
        ...
        if not location or location.cost_per_kg <= 0:
            return make_response({
                "error":"Please enter a valid location"
            },400)
        
        if not number_plate or not capacity or float(capacity) <= 0:
            return make_response({
...
class Locations(Resource):
    ...
    def post(self):
        ...
        if not origin or not destination or float(cost_per_kg) <= 0:
            return make_response({
